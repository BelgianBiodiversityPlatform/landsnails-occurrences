...
#hier komen de distribution erbij, dit moet nog aangepast worden?
```
...
#occurrence %<>% select(-scientificname) #removes column
#why not just rename scientificname to scientificName?
...
#remove collectoin columns
occurrence %<>% select(-c(disposition, etiket_ref, collection))
```
...
write.csv(occurrence, "C:\\Users\\WARD\\OneDrive - UGent\\School\\1ste master\\sem_2\\Project\\landsnails-occurrences\\data\\processed\\occurrence.csv", append = FALSE, na = "",  row.names = FALSE, fileEncoding = "UTF-8")
```
